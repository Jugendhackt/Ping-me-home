 ...
 * Anwendung:
 * const homeCoords = getSavedCoordinates('home');
 * if (homeCoords) console.log(homeCoords.lat, homeCoords.lng);
 */
export function getSavedCoordinates(key: string = 'spawnCoords'): Coordinates | null {
 ...
 * @param key Name/Schlüssel der Koordinaten, default: 'spawnCoords'
 * Anwendung:
 * saveCoordinates({ lat: 52.52, lng: 13.405 }, 'office');
 */
export function saveCoordinates(coords: Coordinates, key: string = 'spawnCoords') {
 ...
 * Anwendung:
 * if (validateCoordinates(52.52, 13.405)) console.log("Gültig");
 */
 ...
 * Anwendung:
 * const coords = await addressToCoordinates("Berlin Alexanderplatz");
 * if (coords) console.log(coords.lat, coords.lng);
 */
export async function addressToCoordinates(addr: string): Promise<Coordinates | null> {
  ...
  return new Promise((resolve) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (!validateCoordinates(lat, lng)) return resolve(null);
        resolve({ lat, lng });
      },
      () => resolve(null)
    );
 ...
 * Anwendung:
 * const result = await setCoordinatesFromInput("Berlin Alexanderplatz");
 * if (result.coords) {
 *   console.log(result.coords.lat, result.coords.lng);
 *   console.log(result.message);
 * }
 */
export async function setCoordinatesFromInput(input: string): Promise<{ coords: Coordinates | null; message: string }> {
  let message = '';
  ...
  coords = await addressToCoordinates(input);
  if (coords) {
